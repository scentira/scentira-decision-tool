...
import { search, publicRisk, queue } from '../lib/domain.ts';
...
test('reseller bulk discounts do not match the customer order-history rule',()=>{assert.deepEqual(search(seedEntries,'A reseller wants a bulk discount.'),[]);assert.equal(search(seedEntries,'A repeat customer wants an additional discount beyond the promotion.')[0].entry.id,'1');});
...
test('founder can add a real precedent directly to the active library',()=>{const action={kind:'createPrecedent',id:'founder-rule-1',title:'Influencer collaboration budget',situation:'An influencer asked what budget we can offer for a collaboration.',decision:'Offer the approved campaign amount.',reasoning:'Keep collaboration spending within the weekly campaign plan.',exception:'Ask the founder when the requested amount is above the campaign limit.',category:'Marketing'};const s=applyAction(fresh(),action,'founder',1234);const entry=s.entries.at(-1);assert.deepEqual(entry,{id:'founder-rule-1',title:action.title,category:'Marketing',situation:action.situation,decision:action.decision,reasoning:action.reasoning,exception:action.exception,status:'Active',seed:false,createdAt:1234});assert.equal(search(s.entries,action.situation)[0].entry.id,'founder-rule-1');assert.equal(applyAction(s,action,'founder',9999).entries.length,s.entries.length);});
test('team and CoS cannot add a precedent directly',()=>{const action={kind:'createPrecedent',id:'blocked-rule',title:'A real decision',situation:'A real situation',decision:'Do this',reasoning:'Because this applies',category:'Operations'};assert.throws(()=>applyAction(fresh(),action,'team'),/Unlock founder or CoS/);assert.throws(()=>applyAction(fresh(),action,'cos'),/Founder access required/);});
