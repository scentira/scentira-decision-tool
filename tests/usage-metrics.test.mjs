...
test('demo match path stops before any database request',()=>{const source=new URL('../components/precedent-app.tsx',import.meta.url);return import('node:fs/promises').then(({readFile})=>readFile(source,'utf8')).then(text=>assert.match(text,/recordRealMatch\(\)\s*\{\s*if \(demo\) return/));});
