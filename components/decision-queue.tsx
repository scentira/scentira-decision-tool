...
export function DecisionQueue({items,renderDetail,onSelect,roleDescription}:{items:QueueItem[];renderDetail:(id:string)=>ReactNode;onSelect:(id:string|null)=>void;roleDescription?:string}) {
 ...
 return <section aria-label="Founder queue"><h1>Founder queue</h1>{roleDescription&&<p className="role-mode-description">{roleDescription}</p>}<p className="intro">One queue for submitted situations and decisions. Pending includes CoS answers awaiting founder review.</p>
