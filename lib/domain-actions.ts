import { AppError, categories, currentRule, publicRisk, type Role, type State, type Situation, type Notice, type Priority, type Category, type Entry } from './domain.ts';
import {fragranceConditions,isFragrancePrecedent} from './fragrance-conditions.ts';
...
export function applyAction(state:State,action:Record<string,unknown>,role:Role,now=Date.now()):State{
 ...
 if(kind==='caseFeedback'){
  const c=s.cases.find(c=>c.id===action.caseId);if(!c?.answer)throw new AppError('Answered case not found.',404);
  if(c.actionIds.includes(id))return s;if(c.callFeedback)throw new AppError('Feedback was already recorded for this case.',409);
  if(!['yes','no','not_sure'].includes(String(action.value)))throw new AppError('Choose Yes, No or Not sure.');
  c.callFeedback={value:action.value as 'yes'|'no'|'not_sure',note:typeof action.note==='string'?action.note.trim().slice(0,1000):'',at:now};c.actionIds.push(id);return s;
 }
 if(kind==='recordMatch'){
  const usage=s.usage||{precedentMatches:0,matchActionIds:[]};
  if(usage.matchActionIds.includes(id))return s;usage.precedentMatches+=1;usage.matchActionIds=[...usage.matchActionIds.slice(-499),id];s.usage=usage;return s;
 }
 if(kind==='conditionApplied'){
  const entry=s.entries.find(e=>e.id===action.precedentId&&e.status==='Active');if(!entry||!isFragrancePrecedent(entry))throw new AppError('Active precedent not found.',404);
  const row=fragranceConditions.find(item=>item.id===action.conditionId);if(!row)throw new AppError('Choose a valid condition.');
  const uses=s.conditionUses||[];if(uses.some(use=>use.id===id))return s;
  uses.push({id,precedentId:entry.id,conditionId:row.id,conditionLabel:row.situation,decision:row.decision,source:'real',createdAt:now});s.conditionUses=uses;return s;
 }
 ...
 if(kind==='createPrecedent'){
  if(role!=='founder')throw new AppError('Founder access required.',403);
  if(s.entries.some(e=>e.id===id))return s;
  if(!categories.includes(action.category as Category))throw new AppError('Choose a category.');
  const situation=text(action.situation,'Situation');
  s.entries.push({id,title:text(action.title,'Title'),category:action.category as Category,situation,decision:text(action.decision,'Decision'),reasoning:text(action.reasoning,'Reasoning'),exception:typeof action.exception==='string'?action.exception.trim():'',status:'Active',seed:false,createdAt:now});
  return s;
 }
 const c=s.cases.find(c=>c.id===action.caseId);
